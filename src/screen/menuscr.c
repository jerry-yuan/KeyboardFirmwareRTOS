...
static HMI_ENGINE_RESULT Initialize(SGUI_SCR_DEV* pstDeviceIF) {
    ...
    menuItems  = pvPortMalloc(sizeof(SGUI_ITEMS_ITEM)*4);
    memset(menuItems,0,sizeof(SGUI_ITEMS_ITEM)*4);
    ...
    menuItems[3].cszLabelText = FA_KEY_SKELETON " Google Authenticator";
    ...
    SGUI_Menu_Initialize(menuObject,&menuRect,&menuPalette,SGUI_FONT_REF(Deng12),menuItems,4);
...
static HMI_ENGINE_RESULT PostProcess(SGUI_SCR_DEV* pstDeviceIF,  HMI_ENGINE_RESULT eProcResult, SGUI_INT iActionID) {
    if(iActionID == GoUp) {
        ...
        }else if(SGUI_Menu_GetSelection(menuObject)->iIndex == 3){
			HMI_SwitchScreen(SCREEN_TOTPAuth_Show_ID,NULL);
        }
    }
