...
HMI_SCREEN_OBJECT SCREEN_USB= {USBScreen_ID,&screenActions};
...
HMI_ENGINE_RESULT Initialize(SGUI_SCR_DEV* pstDeviceIF) {
    if(noticeBox==NULL) {
        ...
        printf("(%d,%d),(%d,%d)\r\n",noticeBox->stLayout.iX,noticeBox->stLayout.iY,noticeBox->stLayout.iWidth,noticeBox->stLayout.iHeight);
...
HMI_ENGINE_RESULT ProcessEvent(SGUI_SCR_DEV* pstDeviceIF,const HMI_EVENT_BASE* pstEvent, SGUI_INT* piActionID) {
    printf("ProcessEvent usb screen!\r\n");
